...
es = Elasticsearch(
    ['https://data.schnitzel.tech:9200'], http_auth=('hack_zurich', 'punctualunicorns'), ssl_context=context
)


@app.route('/recent-query/<query>')
def get_recent_query_data(query):
    results = es.search(index="isp", body={"query": {"match_phrase": {"d": query}}, "sort": {"tm": {"order": "asc"}}})
...
def get_recent_query_data_from_dj(query):
    ...
    result = es.search(
        index="dj",
        body={
            "query": {"query_string": {"query": query, "fields": ['title', 'snippet', 'body']}},
            "size": 10,
            "sort": {"publication_datetime": {"order": "asc"}},
        },
    )
    art = 0
...
def get_past_six_months_trend(query, interval):
    ...
    result = es.search(
        ...
        body={
            "query": {
                "bool": {
                    "must": [{"match": {"snippet": query}}, {"match": {"body": query}}, {"match": {"title": query}}],
                    "filter": {"range": {"publication_datetime": {"gte": "now-6M", "lt": "now"}}},
                }
            },
            "size": 0,
            "aggs": {
                "publication_datetime": {
                    "date_histogram": {"field": "publication_datetime", "interval": interval},
                    "aggs": {"publication_datetime": {"terms": {"field": "publication_datetime"}}},
                }
            },
        },
    )
